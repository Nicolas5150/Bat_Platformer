...
var actorChars = {
  ...
  "s": BatSign, //Collect them all to gain one life
  "a": LevelOne, // Sends you to level one
  "b": LevelTwo, // With access code you can skip to level two
  "c": LevelThree, // With access code you can skip to level three
  "=": acid,
  "|": acid,
  "v": acid
...
// Add a new actor type as a class
function BatSign(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
BatSign.prototype.type = "batSign";

// Add a new actor type as a class
function LevelOne(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
LevelOne.prototype.type = "levelOne";

// Add a new actor type as a class
function LevelTwo(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
LevelTwo.prototype.type = "levelTwo";

// Add a new actor type as a class
function LevelThree(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
LevelThree.prototype.type = "levelThree";
...
BatSign.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

LevelOne.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = 0;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

LevelTwo.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = 0;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

LevelThree.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = 0;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "acid" && this.status == null) {
    lifeCount --;
    document.getElementById('lifeCounter').innerHTML = lifeCount;
    coinCount = 0;
    document.getElementById('coinCounter').innerHTML = coinCount;
    this.status = "lost";
    this.finishDelay = 4;

    // Count down from 3 lives once 0 game starts all the way from level 1
    if (lifeCount == 0)
    {
    ...
    if (!this.actors.some(function(actor) {
      ...
      // Counter for the levels and a checker for when we hit level 4 AKA You've Won
      levelCount ++;
      if (levelCount == 4){
        document.getElementById('levelCounter').innerHTML = "You've Won";
        var filename = "images/batman.jpg";
        document.getElementById('imageResult').innerHTML += "<img src='" + filename + "'>";
      }
      else
      document.getElementById('levelCounter').innerHTML = levelCount;
    }
  }
  ...
  if (type == "batSign") {
    ...
    // If there aren't any parts to the batsign left, player gets extra life
    if (!this.actors.some(function(actor) {
           return actor.type == "batSign";
         })) {
           lifeCount ++;
           document.getElementById('lifeCounter').innerHTML = lifeCount;
  ...
  // Once the touch has been recognized the player gets sent to that level
  if (type == "levelOne") {
